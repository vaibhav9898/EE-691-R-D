...
def train(csf, gen, train_loader, fixed_noise, fixed_input_pdf, cfg, device):
    ...
    for epoch in range(cfg.num_epochs):
        ...
        # Generate input
        noise = torch.randn(cfg.batch_size, cfg.nz, 1, 1).to(device)
        input_pdf = F.softmax(torch.randn(cfg.batch_size, cfg.n_classes).to(device), dim=1)
        ...
        print(f"Epoch {epoch + 1}/{cfg.num_epochs}, "
              f"QL:{quant_loss.item():.4f}, KLD:{kl_divergence_value.item():.4f}, "
        ...
        if (epoch + 1) % 100 == 0:
            with torch.no_grad():
                ...
                vutils.save_image(
                    fixed_gen_images, f"MNIST-Inversion/GF/3/image_{epoch + 1}.png",
